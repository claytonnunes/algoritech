...
if (isset($_REQUEST['acao'])) {
	if ($_REQUEST['acao']=='gerarPdf'){
		...
		pesquisaProdutoTres('id', $_REQUEST['id_grupo_produto'], 'id_pai', $_SESSION['id_pai'], 'deleted', '0');
		if ($grupo_produtos) {
			foreach ($grupo_produtos as $grupo_produto) {
				$nomeNegociacao = $grupo_produtos['nome'];
			}
		}
		...
		pesquisaEdicaoId($_SESSION['id_edicao']);
		if ($eventos) {
...
<?php
	...
	$nome_arquivo = "cotacao".$_REQUEST['id_grupo_produto']."-".$_SESSION['numero_cotacao'].".pdf"; 			
	
	$textoData= "$dia - $mes - $ano\n\n";	
	$textoCodigo= "CODIGO DA COTAÇÃO: ".$_REQUEST['id_grupo_produto']."-".$_SESSION['numero_cotacao']."\nCODIGO DA NEGOCIAÇÃO: ".$nomeNegociacao." \n";	

	$dadosCotacao= "EMPRESA: ".$nomeEmpresa." \nEVENTO: ".$_SESSION['nome_edicao']." \nPERÍODO: ".$periodoEvento." \n\n";
	...
	function header () { 
	$this->Image('../img/upload/relogio_pontual.jpg' ,91 ,10 ,28 ,33 );	
	...
	if ($cotacoes) { 
		...
		foreach ($cotacoes as $cotacao) {
			...
			$valorUnidade = $cotacao['valor_locacao'];
			...
			$pdf->SetX(115);
			$pdf->MultiCell(20,0, $cotacao['quantidade'],0,'J', 0);
